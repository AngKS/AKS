...
module.exports = new Command({
    name : "doggo",
    description : "Generates chunks of random Doggo Lingo placeholder text for you!",
    type : "SLASH",
    slashCommandOptions: [{
        name: "paragraphs",
        description: "Amount of paragraphs of Doggo Ipsum to be generated",
        type: "INTEGER",
        required: true
    }],
    ...
    run : async (message, args, client) => {
        ...
        if (n > 5) return message.reply({content : "My dude, I have a limited vocab of 5 paragraphs!"})
        ...
        let embedCreator = new MessageEmbed()
            ...
            .setAuthor(`Generated for ${message.user.username}`, message.user.displayAvatarURL({ dynamic: true }))
        ...
        const collector = message.channel.createMessageComponentCollector({ time: 15000, max: 5 })
        collector.on('collect', async i => {
            if (i.customId == 'clipboard') {
                clipboardy.writeSync(doggoPara)
                await message.channel.send({ content: "Text successfully copied to clipboard" })
            }
            if (i.customId == 'html') {
                clipboardy.writeSync(`<p>${doggoPara}</p>`)
                await message.channel.send({ content: "HTML successfully copied to clipboard" })
            }
        })

        return await message.channel.send({ embeds: [embedCreator], components: [row] })
